############################################
# Locals (naming convention: ceklab1-*)
############################################
locals {
...
# Explanation: ceklab1 needs a hyperlane—this VPC is the Millennium Falcon’s flight corridor.
resource "aws_vpc" "ceklab1_vpc01" {
...
resource "aws_internet_gateway" "ceklab1_igw01" {
...
resource "aws_subnet" "ceklab1_public_subnets" {
...
resource "aws_subnet" "ceklab1_private_subnets" {
...
# Explanation: ceklab1 wants the private base to call home—EIP gives the NAT a stable “holonet address.”
resource "aws_eip" "ceklab1_nat_eip01" {
  domain = "vpc"
...
# Explanation: NAT is ceklab1’s smuggler tunnel—private subnets can reach out without being seen.
resource "aws_nat_gateway" "ceklab1_nat01" {
  allocation_id = aws_eip.ceklab1_nat_eip01.id
  subnet_id     = aws_subnet.ceklab1_public_subnets[0].id # NAT in a public subnet

  tags = {
    Name = "${local.name_prefix}-nat01"
  }

  depends_on = [aws_internet_gateway.ceklab1_igw01]
}
...
resource "aws_route_table" "ceklab1_public_rt01" {
  vpc_id = aws_vpc.ceklab1_vpc01.id
...
resource "aws_route" "ceklab1_public_default_route" {
  route_table_id         = aws_route_table.ceklab1_public_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.ceklab1_igw01.id
}

# Explanation: Attach public subnets to the “public lanes.”
resource "aws_route_table_association" "ceklab1_public_rta" {
  count          = length(aws_subnet.ceklab1_public_subnets)
  subnet_id      = aws_subnet.ceklab1_public_subnets[count.index].id
  route_table_id = aws_route_table.ceklab1_public_rt01.id
}

# Explanation: Private route table = “stay hidden, but still ship supplies.”
resource "aws_route_table" "ceklab1_private_rt01" {
  vpc_id = aws_vpc.ceklab1_vpc01.id
...
# Explanation: Private subnets route outbound internet via NAT (ceklab1-approved stealth).
resource "aws_route" "ceklab1_private_default_route" {
  route_table_id         = aws_route_table.ceklab1_private_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.ceklab1_nat01.id
}

# Explanation: Attach private subnets to the “stealth lanes.”
resource "aws_route_table_association" "ceklab1_private_rta" {
  count          = length(aws_subnet.ceklab1_private_subnets)
  subnet_id      = aws_subnet.ceklab1_private_subnets[count.index].id
  route_table_id = aws_route_table.ceklab1_private_rt01.id
}
...
# Explanation: ceklab1 refuses to carry static keys—this role lets EC2 assume permissions safely.
resource "aws_iam_role" "ceklab1_ec2_role01" {
...
resource "aws_iam_role_policy_attachment" "ceklab1_ec2_ssm_attach" {
...
resource "aws_iam_role_policy_attachment" "ceklab1_ec2_secrets_attach" {
  role      = aws_iam_role.ceklab1_ec2_role01.name
...
resource "aws_iam_role_policy_attachment" "ceklab1_ec2_cw_attach" {
  role      = aws_iam_role.ceklab1_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

# Explanation: Instance profile is the harness that straps the role onto the EC2 like bandolier ammo.
resource "aws_iam_instance_profile" "ceklab1_instance_profile01" {
...
# Explanation: Parameter Store is ceklab1’s map—endpoints and config live here for fast recovery.
resource "aws_ssm_parameter" "ceklab1_db_endpoint_param" {
...
resource "aws_ssm_parameter" "ceklab1_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.ceklab1_rds01.port)
...
resource "aws_ssm_parameter" "ceklab1_db_name_param" {
  name  = "/lab/db/name"
  type  = "String"
  value = var.db_name
...
resource "aws_cloudwatch_log_group" "ceklab1_log_group01" {
...
# Explanation: Metrics are ceklab1’s growls—when they spike, something is wrong.
# NOTE: Students must emit the metric from app/agent; this just declares the alarm.
resource "aws_cloudwatch_metric_alarm" "ceklab1_db_alarm01" {
  ...
  alarm_actions       = [aws_sns_topic.ceklab1_sns_topic01.arn]
...
resource "aws_sns_topic" "ceklab1_sns_topic01" {
...
resource "aws_sns_topic_subscription" "ceklab1_sns_sub01" {
  topic_arn = aws_sns_topic.ceklab1_sns_topic01.arn
  protocol  = "email"
...
# resource "aws_vpc_endpoint" "ceklab1_vpce_ssm" { ... }
...
# enable_zone_apex_alias   = false   # set true ONLY if you want example.com -> ALB
